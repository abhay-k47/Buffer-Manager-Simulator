...
LRUBufferManager::LRUBufferManager(int numFrames)
    : numFrames(numFrames)
...
BufStats LRUBufferManager::getStats() const
{
  return stats;
}
...
ClockBufferManager::ClockBufferManager(int numFrames)
    : numFrames(numFrames)
    , numPages(0)
...
BufStats ClockBufferManager::getStats() const
{
  return stats;
}
...
char* ClockBufferManager::getPage(FILE* fp, int pageNum)
{
  ...
  while (true)
  {
    ...
    fprintf(file,
            "Buffer Full\t\t\tPage %d of file %p is removed from buffer\n",
            bufferPool[clock_hand].pageNum,
            bufferPool[clock_hand].fp);
    fseek(fp, pageNum * PAGE_SIZE, SEEK_SET);
...
MRUBufferManager::MRUBufferManager(int numFrames)
    : numFrames(numFrames)
    , file(fopen("bufferManager.log", "w"))
{
...
BufStats MRUBufferManager::getStats() const
{
  return stats;
}
